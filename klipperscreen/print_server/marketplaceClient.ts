/**
 * WebSocket client that connects this printer server to the marketplace backend.
 * Identical in structure to x402_transactions/printer-server/marketplaceClient.ts
 * but adapted for the klipperscreen variant (Moonraker-aware app).
 *
 * Flow:
 *   1. Connect to wss://<MARKETPLACE_URL>/ws/printer
 *   2. Send {"type":"register","token":"...","info":{…}}
...
export function connectMarketplace(cfg: AppConfig, marketplaceUrl: string, token: string, app: Hono): void {
  ...
  function connect(): void {
    ...
    ws.onclose = () => {
      ...
      setTimeout(() => { delay = Math.min(delay * 2, MAX_DELAY_MS); connect(); }, delay);
    };
